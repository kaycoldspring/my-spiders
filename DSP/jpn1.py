...
myclient = MongoClient(host='dds-6we5ef24527af8e41.mongodb.japan.rds.aliyuncs.com',port=3717,username='root',password='3dIrM#sA@MA$x4aKGB',
...
def send_requests(device_offer_id, ip, port, url, ua, device_ip, id):
    if device_offer_id:
        t0 = time.time()
        headers = {
            'User-Agent': ua,
        ...
        if len(link_list) > 0:
            ...
            last_link = url
        t = time.time()
        print('current IP:',ip, '    redirectNum:',redirectNum,'    offer_id:',id,'      used time:',t-t0,'    last_link:',last_link,)
        return 1
    else:
...
def get_detect_resp(offer_info):  # 参数为offer信息,是一个数�?
    ...
    # print('ip:',ip, port)
    # t3 = time.time()
    link_status = send_requests(device_offer_id, ip, str(port), template_url, ua, device_ip,id)
    # t4 = time.time()
...
def detect_offer(line):
    ...
    if needexe == 1:
        try:
            detect_list = [[d[0], d[1], d[2], d[3], d[4], ifa, d[5], ua, device_ip] for d in offers_info if d[0] not in detected_offer]
...
def get_offer_info(country):
    ...
    cursor = db.cursor()
    while 1:
        sql = "select offer_id,platform,country,tracking_link,app_id,id from offer where aff_id = 215 and status = 1 and " \
...
if __name__ == '__main__':
    ...
    gt = parse(rq + ' 00:00:00')
    lt = parse(rq + ' 06:00:00')
    while 1 :
            ...
            with open('/data/work/'+ files[0]) as f:
                while 1:
                    ...
                    if NOW > gt and NOW < lt:
                        ...
                        if line == '':
                            break
                        if '"JPN"' in line and 'Android' in line:
