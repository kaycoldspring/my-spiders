...
import time
import execjs
from 百度企业信用.scrapy_request.items import BusinessItem, XinbdItem
from util.proxies import Proxies
from util.db import get_redis_cli
from scrapy.spidermiddlewares.httperror import HttpError
from threading import Event
# twisted 异常捕获
from twisted.internet.error import ConnectionLost, TCPTimedOutError, TimeoutError, DNSLookupError
...
class BAIDU_SPIDER(RedisSpider):
	...
	custom_settings = dict(
		...
		AUTOTHROTTLE_DEBUG=False,   	# 启用 AutoThrottle 调试模式，该模式显示收到的每个响应的统计数据，以便可以实时调节参数
	...
	def errback_twisted(self, failure):
		if failure.check(TimeoutError, TCPTimedOutError, DNSLookupError):
			...
			self.server.sadd(self.fail_queue, company_name)
	...
	def start_requests(self):
		...
		while True:
			...
			if company_name:
				...
				yield scrapy.Request(uri, headers=self.headers, meta={"company_name": company_name}, callback=self.parse)
			else:
				break
	...
	def parse(self, response):
		...
		if flg:
			...
			yield scrapy.Request(uri, headers=self.headers, callback=self.get_result_url, meta=response.meta)
		else:
			# 没有信息的公司,进行保存
	...
	def get_result_url(self, response):
		...
		# 请求上述 url, 获取到数据入库
		yield scrapy.Request(uri, headers=self.headers, callback=self.parse_result, meta=response.meta)
